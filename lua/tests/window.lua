...
local renderer =
  require 'livetablelogger/renderer'
...
function Window:new(opts)
  ...
  opts.border.thickness =
    opts.border.thickness or {}
  ...
  custom_opts = {
    ...
    grow_side = get_default(opts.grow_size,
                            'smart'),
    ...
    custom_opts.obj_padding =
      {
        ...
        right = get_default(
          opts.obj_padding.right, 1),
        bottom = get_default(
          opts.obj_padding.bottom, 1),
        left = get_default(opts.obj_padding.left,
                           1)
      }
  end
    ...
    custom_opts.log_padding =
      {
        ...
        right = get_default(
          opts.log_padding.right, 1),
        bottom = get_default(
          opts.log_padding.bottom, 1),
        left = get_default(opts.log_padding.left,
                           1)
      }
  end

  opts.border.borderchars =
    get_default(opts.border.borderchars, {
      '─', '│', '─', '│', '╭', '╮',
      '╯', '╰'
    })
  ...
  opts.border.obj_border_opts =
    {
      title = get_default(opts.border.obj_title,
                          'obj'),
      border_thickness = {
        top = get_default(
          opts.border.thickness.top, 1),
        right = get_default(
          opts.border.thickness.right, 1),
        bot = get_default(
          opts.border.thickness.bot, 1),
        left = get_default(
          opts.border.thickness.left, 1)
      }
    }

  opts.border.log_border_opts =
    {
      title = get_default(opts.border.log_title,
                          'log'),
      border_thickness = {
        top = get_default(
          opts.border.thickness.top, 1),
  ...
  obj_border_opts = vim.tbl_extend('keep',
                                   borderchars_kv,
                                   opts.border
                                     .obj_border_opts)
  log_border_opts = vim.tbl_extend('keep',
                                   borderchars_kv,
                                   opts.border
                                     .log_border_opts)
  ...
  assert(total_opts.width <= 1,
         'LTL ERROR: view width 0 - 1 number supported only')
  assert(total_opts.height <= 1,
         'LTL ERROR: view height 0 - 1 number supported only')
  assert(total_opts.width > 0,
         'LTL ERROR: view width needs to be 0 - 1 number')
  assert(total_opts.height > 0,
         'LTL ERROR: view height needs to be 0 - 1 number')

  -- decide max_col, max_row dependant on win or global, for pin
  if total_opts.relative == 'win' then
    local cwin_max_col = vim.api
                           .nvim_call_function(
                           'winwidth', { 0 })
    local cwin_max_row = vim.api
                           .nvim_call_function(
                           'winheight', { 0 })
  ...
  -- always center
  local col_start_percentage =
    (1 - total_opts.width) / 2
  local row_start_percentage =
    (1 - total_opts.height) / 2

  total_opts.height = math.floor(
                        custom_opts.max_row
                          * total_opts.height)
  total_opts.width = math.floor(
                       custom_opts.max_col
                         * total_opts.width)
  total_opts.col = custom_opts.max_col
                     * col_start_percentage
  total_opts.row = custom_opts.max_row
                     * row_start_percentage

  local function apply_pin(colrow,
                           total_opts,
                           custom_opts)

    if custom_opts.pin == nil then return false end

    local max_col, max_row = custom_opts.max_col,
                             custom_opts.max_row
    local width, height = total_opts.width,
                          total_opts.height

    local choose_pin = {
      top = ternary(colrow, false, 0),
      topright = ternary(colrow, max_col - width,
                         0),
      right = ternary(colrow, max_col - width,
                      false),
      bottomright = ternary(colrow,
                            max_col - width,
                            max_row - height),
      bottom = ternary(colrow, false,
                       max_row - height), -- -1 for statusline
      bottomleft = ternary(colrow, 0,
                           max_row - height), -- -1 for statusline
  ...
  -- then apply pin
  total_opts.col = apply_pin(true, total_opts,
                             custom_opts)
                     or total_opts.col
  total_opts.row = apply_pin(false, total_opts,
                             custom_opts)
                     or total_opts.row
  ...
  local function create_window_opts(total_opts,
                                    custom_opts,
                                    objlog,
                                    layout_bool)
  ...
  obj_opts = create_window_opts(total_opts,
                                custom_opts, true,
                                layout_bool)
  log_opts = create_window_opts(total_opts,
                                custom_opts,
                                false, layout_bool)
  ...
  if layout_bool and gap_bool then
    obj_opts.width = obj_opts.width
                       - custom_opts.gap
  elseif gap_bool then
    obj_opts.height = obj_opts.height
                        - custom_opts.gap
  end
  ...
  if custom_opts.layout == 'horizontal' then
    ...
    local row_to_move = math.floor(
                          total_opts.height
                            * distance)
    obj_opts.height =
      obj_opts.height - row_to_move
    log_opts.row = log_opts.row - row_to_move
    log_opts.height =
      log_opts.height + row_to_move
  end

  local function apply_padding(win_opts,
                               custom_opts,
                               objorlog)
    local objorlog_padding =
      custom_opts[objorlog .. '_padding']
  ...
  obj_opts = apply_padding(obj_opts, custom_opts,
                           'obj')
  log_opts = apply_padding(log_opts, custom_opts,
                           'log')
...
function Window:open()
  ...
  self.total_opts.win = vim.api
                          .nvim_get_current_win()
  if self.total_opts.relative == 'win' then
    self.obj_opts.win = vim.api
                          .nvim_get_current_win()
  end
  if self.total_opts.relative == 'win'
    and self.log_opts then
    self.log_opts.win = vim.api
                          .nvim_get_current_win()
  end
  ...
  local function open_window(bufnr,
                             ...
                             custom_opts,
                             objorlog)
    local objorlog_winnr =
      vim.api.nvim_open_win(bufnr, false,
                            popup_opts)

    vim.api.nvim_win_set_option(winnr, 'winblend',
                                15)
  ...
  if self.custom_opts.log then
    self.obj_winnr, self.obj_border_winnr =
      open_window(self.obj_bufnr, self.obj_opts,
                  self.custom_opts, 'obj')
    self.log_winnr, self.log_border_winnr =
      open_window(self.log_bufnr, self.log_opts,
                  self.custom_opts, 'log')
  else
    self.obj_winnr, self.obj_border_winnr =
      open_window(self.obj_bufnr, self.total_opts,
                  self.custom_opts, 'obj')
  end
...
function windows.open(opts)
  ...
  local function parse_view_opts(opts)
    ...
    for k, v in pairs(opts or {}) do
      if k:find('^view[%d]*') then
        table.insert(views, k)
      end
    end
    if vim.tbl_isempty(views) then
      return false
    else
      return views
    end
  ...
  view_in_keys = parse_view_opts(opts)
                   or { 'view1' }
  ...
  for _, view in ipairs(view_in_keys) do
    ...
    assert(target_exists(view, state),
           'LTL ERROR: cant find target in active instances')

    if not state.ui[view] then
      state.ui[view] =
        Window:new(opts[view] or {})
    end
    state.ui[view]:open()
  end
